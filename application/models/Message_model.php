...
class Message_model extends CI_Model {
     ...
     */
    function get_all($to_id, $limit=0, $offset=0, $filters=array(), $sort='datetime', $dir='DESC')
    {
        // start building query
        $sql = "
            SELECT SQL_CALC_FOUND_ROWS m.*, 
            u1.first_name as from_fn, 
            u1.last_name as from_ln, 
            u2.first_name as to_fn, 
            u2.last_name as to_ln 
            FROM {$this->_db} m 
            LEFT JOIN users u1 ON u1.id = m.from_id 
            LEFT JOIN users u2 ON u2.id = m.to_id
            WHERE m.to_id = {$to_id}
        ";
